...
void Truss2D::calculate_kk_element(size_t e) {
    ...
    double c = (xb - xa) / l;
    double s = (yb - ya) / l;
    double p = (divide_property_by_length ? properties[e] / l : properties[e]);

    // TODO: compute upper triangle only
    ...
    kk_element->set(1, 0, kk_element->get(0, 1));
    kk_element->set(1, 1, p * s * s);
    kk_element->set(1, 2, kk_element->get(0, 3));
    kk_element->set(1, 3, -p * s * s);

    kk_element->set(2, 0, kk_element->get(0, 2));
    kk_element->set(2, 1, kk_element->get(0, 3));
    kk_element->set(2, 2, kk_element->get(0, 0));
    kk_element->set(2, 3, kk_element->get(0, 1));

    kk_element->set(3, 0, kk_element->get(0, 3));
    kk_element->set(3, 1, kk_element->get(1, 3));
    kk_element->set(3, 2, kk_element->get(0, 1));
    kk_element->set(3, 3, kk_element->get(1, 1));
}
